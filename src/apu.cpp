...
uint8_t APU::length_table[] = {  10, 254, 20,  2, 40,  4, 80,  6,
    ...
    switch (addr) {
        ...
        case 0x400e:
            switch (data & 0x0f) {
                ...
                case 0x09: noise_seq.reload = 292; break;
...
void APU::clock() {
    // depending on frame count, we set a flag  to tell where we are i nsequence
    // changes to notes only occur at these intervals
    // menaing this is responsible for ensuring musical time is maintained
    bool bQuarterFrameClock = false;
    bool bHalfFrameClock = false;

    dGlobalTime += (0.33333333 / 1789773);
    ...
    if (clock_counter % 6 == 0) {
        ...
        // update func units
        // quarter frame "beats" adjust the volume evenlope
        if (bQuarterFrameClock) {
            pulse1_env.clock(pulse1_halt);
            pulse2_env.clock(pulse2_halt);
            noise_env.clock(noise_halt);
        }
        // half frame beats adjust the note length and freq sweepers
        if (bHalfFrameClock) {
        ...
        // if (bUseRawMode) {
            ...
            pulse1_seq.clock(pulse1_enable, [](uint32_t &s) {
                // shift right one bit, wrap around
            ...
            // pulse2_sample = (double) pulse1_seq.output;
        // } else {
    ...
    // frewq sweepers change at high freq
